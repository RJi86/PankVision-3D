...
def train(model, data_in, loss, optim, max_epochs, model_dir, test_interval=1, device=torch.device("cuda:0")):
    ...
    for epoch in range(max_epochs):
        ...
        print(f'Train metric standard deviation: {train_metric_std:.4f}')

        save_loss_std_train.append(train_loss_std)
        np.save(os.path.join(model_dir, 'loss_std_train.npy'), save_loss_std_train)
